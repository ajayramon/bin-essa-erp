...
const PRESET_USERS: Record<string, PresetUser> = {
  admin: {
    id: "u-001",
    username: "admin",
    fullName: "Ahmad Al-Bin Essa (Admin)",
    role: "ADMIN",
    branchId: null,
  },
  ahmad: {
  ...
  try {
    ...
    if (backendUrl && backendUrl !== "/api" && !backendUrl.includes("localhost")) {
      try {
        ...
        if (backendRes.ok) {
          ...
          response.cookies.set("bin_essa_token", data.access_token, {
            path: "/",
            maxAge: 86400 * 7,
            sameSite: "lax",
          });
    ...
    if (!user) {
      ...
      if (lowerUser.includes("cashier") || lowerUser.includes("pos")) {
        ...
        lowerUser.includes("store") ||
        lowerUser.includes("stock") ||
        lowerUser.includes("ware")
      ) {
        ...
        lowerUser.includes("b2b") ||
        lowerUser.includes("client") ||
        lowerUser.includes("wholesale")
      ) {
        assignedRole = "B2B_CUSTOMER";
      }
