...
import { listPurchaseOrdersRequest, type PurchaseOrderResponse } from "@/lib/api";
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-ink/60">
            View all posted purchase orders, supplier bills, and auto-posted inventory journal entries.
          </p>
        </div>
        <Link
      ...
      {!isLoading && !error && (
        <div className="overflow-x-auto rounded-2xl border border-ink/10 bg-white shadow-sm">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-ink/10 text-xs font-semibold uppercase tracking-wider text-ink/50">
                <th className="px-4 py-3 text-start">PO Number</th>
            ...
            <tbody className="divide-y divide-ink/5">
              {orders.map((po) => (
                <tr key={po.id} className="hover:bg-ink/5">
                  ...
                  <td className="px-4 py-3 text-ink/70">{po.supplier?.name || po.supplierId}</td>
          ...
          {orders.length === 0 && (
            <p className="px-4 py-8 text-center text-sm text-ink/40">No purchase orders found.</p>
          )}
        </div>
      )}
    </div>
